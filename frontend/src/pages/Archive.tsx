...
const Archive = () => {
  ...
  const fetchFiles = useCallback(async (page: number = 1) => {
    try {
      ...
      const response = await fetch(`${API_URL}/library/archived?${params}`, {
        headers: {
      ...
      if (data.success) {
        ...
        setError(data.error || 'Failed to fetch archived files');
      }
    } catch (err) {
  ...
  // Handle restore file (set isActive: true)
  const handleRestore = async (e: React.MouseEvent, file: LibraryFile) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        toast.success(`"${file.title}" has been restored to the library`);
  ...
  // Handle delete file permanently
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        <ArchiveBoxIcon className="h-16 w-16 text-gray-300 dark:text-gray-600" />
  ...
  return (
    <div className="space-y-8 min-w-0 max-w-full">
      {/* Header Section */}
      <div className="bg-gradient-to-r from-amber-500 to-amber-600 rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-3 text-lg text-amber-100">
                View and manage archived files from the library
              </p>
            </div>
            <Link
              to="/library"
              className="mt-4 sm:mt-0 inline-flex items-center px-4 py-2 bg-white text-amber-600 font-medium rounded-lg hover:bg-amber-50 transition-colors duration-200"
            >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1 min-w-0 flex gap-2">
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-amber-500/75 hover:bg-amber-600/75 text-white font-medium rounded-lg shadow-sm hover:shadow transition-all duration-200 flex items-center gap-2"
            >
          ...
          <div className="flex gap-2 flex-shrink-0">
            <div className="w-40 flex-shrink-0">
              <CustomSelect
                ...
                options={[
                  { value: '-updatedAt', label: 'Recently Archived' },
            ...
            <div className="flex rounded-lg border border-gray-300 dark:border-gray-600 overflow-hidden">
              <button
                ...
                className={`p-2 ${viewMode === 'grid' ? 'bg-amber-500/75 text-white' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-600'} transition-colors duration-200`}
              >
              ...
              <button
                ...
                className={`p-2 ${viewMode === 'list' ? 'bg-amber-500/75 text-white' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-600'} transition-colors duration-200`}
              >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 flex items-center">
            <FolderSolidIcon className="h-5 w-5 mr-2 text-gray-500 dark:text-gray-400" />
            Archived Files
        ...
        <div className="p-6">
          {error ? (
            ...
            <div className="text-center py-12">
              <ArchiveBoxIcon className="h-16 w-16 mx-auto text-gray-300 dark:text-gray-600" />
              <p className="mt-4 text-gray-500 dark:text-gray-400">No archived files</p>
              <Link
                to="/library"
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {files.map((file) => (
                  <div
                    key={file._id}
                    className="relative border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:border-gray-400 dark:hover:border-gray-500 hover:shadow-md transition-all duration-200 cursor-pointer group bg-gray-50 dark:bg-gray-900/50"
                  ...
                  >
                    ...
                    <div className="mt-3 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <span className="text-gray-400">Archived {formatDate(file.updatedAt)}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <>
              <div className="space-y-2">
                {files.map((file) => (
                  <div
                    key={file._id}
                    className="flex items-center justify-between p-3 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg cursor-pointer group bg-gray-50 dark:bg-gray-900/50"
                    onClick={() => handlePreview(file)}
                  >
                    <div className="flex items-center flex-1 min-w-0">
                      {getFileIcon(file.fileType)}
                      <div className="ml-3 flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mt-1">
                          ...
                          <span>•</span>
                          <span>Archived {formatDate(file.updatedAt)}</span>
                        </div>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Are you sure you want to permanently delete <span className="font-medium text-gray-900 dark:text-gray-100">"{fileToDelete.title}"</span>? This action cannot be undone.
            </p>
