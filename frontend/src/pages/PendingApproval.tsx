...
const PendingApproval = () => {
  ...
  const fetchFiles = useCallback(async (page: number = 1) => {
    try {
      ...
      const response = await fetch(`${API_URL}/library/pending?${params}`, {
        headers: {
      ...
      if (data.success) {
        ...
        setError(data.error || 'Failed to fetch pending files');
      }
    } catch (err) {
  ...
  // Handle approve file
  const handleApprove = async (e: React.MouseEvent, file: LibraryFile) => {
    e.stopPropagation();
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/library/${file._id}/approve`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      const data = await response.json();
      if (data.success) {
        toast.success(`"${file.title}" has been approved and is now visible in the library`);
        fetchFiles(currentPage);
      } else {
        toast.error(data.error || 'Failed to approve file');
      }
    } catch (err) {
  ...
  // Open reject modal
  const openRejectModal = (e: React.MouseEvent, file: LibraryFile) => {
    e.stopPropagation();
  ...
  const handleReject = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/library/${fileToReject._id}/reject`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const data = await response.json();
      if (data.success) {
        toast.success(`"${fileToReject.title}" has been rejected`);
        setShowRejectModal(false);
        setFileToReject(null);
        setRejectReason('');
        fetchFiles(currentPage);
      } else {
        toast.error(data.error || 'Failed to reject file');
      }
    } catch (err) {
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        <ClipboardDocumentListIcon className="h-16 w-16 text-gray-300 dark:text-gray-600" />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-3 text-lg text-blue-100">
                Review and approve files uploaded by owners
              </p>
            </div>
            <Link
              to="/library"
              className="mt-4 sm:mt-0 inline-flex items-center px-4 py-2 bg-white text-blue-600 font-medium rounded-lg hover:bg-blue-50 transition-colors duration-200"
            >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1 min-w-0 flex gap-2">
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-lg shadow-sm hover:shadow transition-all duration-200 flex items-center gap-2"
            >
          ...
          <div className="flex gap-2 flex-shrink-0">
            ...
            <div className="flex rounded-lg border border-gray-300 dark:border-gray-600 overflow-hidden">
              <button
                ...
                className={`p-2 ${viewMode === 'grid' ? 'bg-blue-500 text-white' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-600'} transition-colors duration-200`}
              >
              ...
              <button
                ...
                className={`p-2 ${viewMode === 'list' ? 'bg-blue-500 text-white' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-600'} transition-colors duration-200`}
              >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 flex items-center">
            <FolderSolidIcon className="h-5 w-5 mr-2 text-blue-500" />
            Pending Files
        ...
        <div className="p-6">
          {error ? (
            ...
            <div className="text-center py-12">
              <ClipboardDocumentListIcon className="h-16 w-16 mx-auto text-gray-300 dark:text-gray-600" />
              <p className="mt-4 text-gray-500 dark:text-gray-400">No pending files to review</p>
              <p className="mt-2 text-sm text-gray-400 dark:text-gray-500">Files uploaded by owners will appear here for approval</p>
              <Link
                to="/library"
                  ...
                  >
                    ...
                    <div className="mt-3 flex gap-2">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg hover:bg-green-200 dark:hover:bg-green-900/50 transition-colors text-sm font-medium"
                      >
                        <CheckCircleIcon className="h-4 w-4" />
                        Approve
                      </button>
                      <button
                        onClick={(e) => openRejectModal(e, file)}
                        className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors text-sm font-medium"
                      >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Reject File</h3>
            </div>
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Reject <span className="font-medium text-gray-900 dark:text-gray-100">"{fileToReject.title}"</span> uploaded by {fileToReject.uploadedBy?.firstName} {fileToReject.uploadedBy?.lastName}?
            </p>
            <div className="mb-6">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-red-500 focus:border-red-500"
              />
            </div>
            <div className="flex gap-3">
              ...
              >
                {rejecting ? 'Rejecting...' : 'Reject File'}
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center min-w-0 flex-1">
              ...
              <div className="ml-2 min-w-0">
                ...
                <p className="text-sm text-gray-400 truncate">
                  {formatFileSize(previewFile.fileSize)} • {previewFile.fileName} • Uploaded by {previewFile.uploadedBy?.firstName} {previewFile.uploadedBy?.lastName}
                </p>
              </div>
            </div>
